...
p <- add_option(object = p, opt_str = c("--snp_file"), default = NULL, type = "character",
    help = "[REQUIRED] A tab delimited file containing N + 1 rows and V + 1 columns, where N is the number of samples, and V is the number of DNA variants. The first row and column must contain sample IDs and variants IDs respectively. The column sample ID order should match --omic_data_file.")
