...
p <- add_option(object = p, opt_str = c("--omic_data_file"), default = NULL, type = "character",
    help = "[REQUIRED] A tab delimited file containing N + 1 columns and G + 1 rows, where N is the number of samples, and G is the number of features (genes, methylation sites, chromatin accessibility windows, etc.). The first row and column must contain feature IDs and sample IDs respectively. Feature values should be normalized across samples.")
p <- add_option(object = p, opt_str = c("--snp_file"), default = NULL, type = "character",
    help = "[REQUIRED] A tab delimited file containing N + 1 columns and V + 1 rows, where N is the number of samples, and V is the number of DNA variants. The first row and column must contain variant IDs and sample IDs respectively. The column sample ID order should match --omic_data_file.")
